...
def main() -> int:
    ...
    if not matched_list:
        raise RuntimeError("Cannot find {} in {}".format(args.test, args.testlist))

    # Create seed generator to correlate seeds with run.py
    seed_gen = SeedGen(args.start_seed, None, None)
    ...
    for test in matched_list:
        ...
        if args.batch_size > 0:
            batch_cnt = int((iterations + args.batch_size - 1) / args.batch_size)
        for i in range(0, batch_cnt):
            seed = seed_gen.get(None, i * batch_cnt)
