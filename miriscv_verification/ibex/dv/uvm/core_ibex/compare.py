...
def compare_test_run(test: TestEntry,
    ...
    compare_result = \
        compare_trace_csv(rtl_trace_csv, iss_csv, "DUT", iss, compare_log,
...
def main() -> int:
    ...
    entry = get_test_entry(testname, args.testlist)
    ...
    if args.batch_size > 0:
        batch_cnt = int((entry['iterations'] + args.batch_size - 1) / args.batch_size)

    iteration = int((seed - args.start_seed) / batch_cnt)
