...
def compare_trace_csv(csv1, csv2, name1, name2, log,
    ...
    with open(csv1, "r") as fd1, open(csv2, "r") as fd2:
        ...
        if in_order_mode:
            ...
            # Check only final states of GPR
            if compare_final_value_only:
                fd.write("\nFinal GPR state compare:\n")
                # GPR change dictionaries
                gpr_val_1 = {}
                gpr_val_2 = {}
                # Obtain all regiters changes
                for trace in instr_trace_1:
                    check_update_gpr(trace.gpr, gpr_val_1)
                for trace in instr_trace_2:
                    check_update_gpr(trace.gpr, gpr_val_2)
                # Compare registers
                mismatch_cnt = 0
                for rd in gpr_val_1:
                    if(gpr_val_1[rd] != gpr_val_2[rd]):
                        fd.write("Mismatch for {}:\n{}[{}] :{}\n".format(
                            rd, name1, rd, gpr_val_1[rd]))
                        fd.write("{}[{}] : {}\n".format(
                            name2, rd, gpr_val_2[rd]))
                        mismatch_cnt += 1
                # Calculate match count
                matched_cnt = len(gpr_val_1) - mismatch_cnt
            else:
                pass
